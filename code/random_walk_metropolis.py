...
def rwm(log_density, n_its, x_0, cov, gen, bar=False):
    ...
    for n in nrange(n_its, bar):
        ...
        if np.log(u) <= ldv_nprop - ldv[n-1]:
            ...
            X[n] = X[n-1]
            ldv[n] = ldv[n-1]
    return X, gen, np.ones(n_its+1), ldv
    ...
    for n in nrange(n_its, bar):
        ...
        if n >= 2*d:
            try:
                ...
                L = None
    return X, gen, np.ones(n_its+1), ldv, m, Sig
...
def parallel_ada_rwm(
    ...
    """An implementation of the adaptive random walk Metropolis algorithm
        ...
        Returns:
            samples: np array of shape (n_chains, n_its+1, d), where 
                samples[i,j] is the j-th sample generated by the i-th chain
            ldv: np array of shape (n_its+1,n_chains) giving the log density
    ...
    ldvs = np.array([ret[3] for ret in returns]).T
    means = np.transpose(np.array([ret[4] for ret in returns]), axes=(1,0,2))
    covs = np.array([ret[5] for ret in returns])
    return samples, ldvs, means, covs
